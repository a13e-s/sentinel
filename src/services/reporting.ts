 ...
 * Combines specialist deliverables into a final security assessment report.
 */
...
export async function assembleFinalReport(sourceDir: string, logger: ActivityLogger): Promise<string> {
  ...
  for (const file of deliverableFiles) {
    ...
    try {
      ...
      logger.warn(`Could not read ${file.path}: ${errorMessage}`);
    }
  }
  ...
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    throw new PentestError(
      `Failed to write final report: ${errorMessage}`,
      'filesystem',
      false,
      { finalReportPath, originalError: errorMessage },
      undefined,
...
export async function injectModelIntoReport(
  ...
  if (match) {
    ...
    // 5. If no Assessment Date line found, try to add after Executive Summary header
