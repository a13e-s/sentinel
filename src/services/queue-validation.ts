/**
 * Queue validation for exploitation phase decisions.
 *
 * Pure domain logic: reads queue file, parses JSON, returns decision.
...
/** Validate queue and deliverable files for exploitation decision. */
...
/** Safely validate queue and deliverable files. Returns Result for explicit error handling. */
export async function validateQueueSafe(
  ...
  try {
    ...
    return err(error instanceof PentestError ? error : new PentestError(
