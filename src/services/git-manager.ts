/**
 * Git workspace management for agent execution.
 * Handles checkpoints, rollbacks, and success commits with
 * lock conflict retry and parallel-safe serialization.
 */
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (isGitLockError(errMsg) && attempt < maxRetries) {
          ...
          const warnMsg = `Git lock conflict during ${description} (attempt ${attempt}/${maxRetries}). Retrying in ${delay}ms...`;
          if (logger) {
            logger.warn(warnMsg);
          } else {
...
export async function rollbackGitWorkspace(
  ...
  try {
    const changes = await getChangedFiles(sourceDir, 'status check for rollback', logger);
...
export async function createGitCheckpoint(
  ...
  try {
    ...
    const changes = await getChangedFiles(sourceDir, 'status check', logger);
    const hasChanges = changes.length > 0;

    // 3. Stage and commit checkpoint
    await executeGitCommandWithRetry(['git', 'add', '-A'], sourceDir, 'staging changes', 5, logger);
    await executeGitCommandWithRetry(
...
/** Commit successful agent results. */
export async function commitGitSuccess(
  ...
  try {
    const changes = await getChangedFiles(sourceDir, 'status check for success commit', logger);
...
/** Get current git commit hash. Returns null if not a git repository. */
