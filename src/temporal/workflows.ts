/**
 * Temporal workflow for Sentinel pentest pipeline.
 *
...
export async function pentestPipelineWorkflow(
  ...
  if (input.resumeFromWorkspace) {
    ...
    const incompleteAgents = ALL_AGENTS.filter(
      (agentName) => !loadedResumeState.completedAgents.includes(agentName)
    ) as AgentName[];
  ...
  try {
    ...
    // NOTE: Default reduced from 5 to 2 to stay within Anthropic's 450K
    // input tokens/min rate limit. Override via pipelineConfig if needed.
    const maxConcurrent = input.pipelineConfig?.max_concurrent_pipelines ?? 2;
