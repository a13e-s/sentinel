...
/**
 * Temporal client for starting Sentinel pentest pipeline workflows.
 *
 ...
 *   --workflow-id <id>    Custom workflow ID (default: sentinel-<timestamp>)
...
function parseCliArgs(argv: string[]): CliArgs {
  ...
  return {
    ...
    ...(displayOutputPath != null && { displayOutputPath }),
    ...(customWorkflowId != null && { customWorkflowId }),
    ...(resumeFromWorkspace != null && { resumeFromWorkspace }),
  };
}
...
async function resolveWorkspace(
  ...
  if (!args.resumeFromWorkspace) {
    ...
    const workflowId = args.customWorkflowId || `${hostname}_sentinel-${Date.now()}`;
    return {
      workflowId,
  ...
  return {
    workflowId: `${workspace}_sentinel-${Date.now()}`,
    sessionId: workspace,
    isResume: false,
    terminatedWorkflows: [],
  };
...
async function loadPipelineConfig(configPath: string | undefined): Promise<PipelineConfig> {
  ...
  try {
    // Sentinel's parseConfig takes a YAML string, not a file path
...
function buildPipelineInput(
  ...
  return {
    ...
    ...(args.configPath != null && { configPath: args.configPath }),
    ...(args.outputPath != null && { outputPath: args.outputPath }),
    ...(args.pipelineTestingMode && { pipelineTestingMode: args.pipelineTestingMode }),
    ...(workspace.isResume && args.resumeFromWorkspace != null && { resumeFromWorkspace: args.resumeFromWorkspace }),
...
function displayMonitoringInfo(args: CliArgs, workspace: WorkspaceResolution): void {
  ...
  console.log(`  Logs:    ./sentinel logs ID=${workspace.workflowId}`);
  console.log();
  console.log('Output:');
