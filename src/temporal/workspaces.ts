...
/**
 * Workspace listing tool for Sentinel.
 *
...
async function listWorkspaces(): Promise<void> {
  ...
  if (workspaces.length === 0) {
    console.log('\nNo workspaces found.');
    console.log('Run a pipeline first: ./sentinel start URL=<url> REPO=<repo>');
    return;
  }
  ...
  if (resumableCount > 0) {
    console.log('\nResume with: ./sentinel start URL=<url> REPO=<repo> WORKSPACE=<name>');
  }
