/**
 * Temporal activities for Sentinel agent execution.
 *
...
import { PentestError, ErrorCode } from '../types/errors.js';
...
import type { TraceContext } from '../types/metrics.js';
...
import { AGENTS, resolveModelConfig } from '../session-manager.js';
import { executeGitCommandWithRetry } from '../services/git-manager.js';
import type { ResumeAttempt } from '../audit/metrics-tracker.js';
import { createActivityLogger } from './activity-logger.js';
import { validateProvider } from '../services/preflight.js';
...
async function runAgentActivity(
  ...
  try {
    ...
    const traceContext: TraceContext = {
      workflowId,
      runId: `${workflowId}-${agentName}-${attemptNumber}`,
      agentName,
      attemptNumber,
    };

    logger.info('Agent activity started', { ...traceContext });
    ...
    // Check if output validation retry limit reached
    if (
 ...
 * 3. Provider credential validation via lightweight HTTP probe
 */
export async function runPreflightValidation(input: ActivityInput): Promise<void> {
  ...
  try {
    const logger = createActivityLogger();
    logger.info('Running preflight validation...', { workflowId: input.workflowId, attemptNumber });

    // 1. Validate repository path
    const repoExists = await fileExists(input.repoPath);
    if (!repoExists) {
      throw ApplicationFailure.nonRetryable(
        `Repository not found: ${input.repoPath}`,
        'ConfigurationError',
        [{ phase: 'preflight', attemptNumber }]
      );
    }

    // 2. Validate config file if provided
    if (input.configPath) {
      const configExists = await fileExists(input.configPath);
      if (!configExists) {
        throw ApplicationFailure.nonRetryable(
    ...
    // 3. Resolve model config and validate provider credentials
    const sessionMetadata = buildSessionMetadata(input);
    const container = getOrCreateContainer(input.workflowId, sessionMetadata);
    const rawConfigResult = await container.configLoader.loadRawOptional(input.configPath);
    const rawConfig = isErr(rawConfigResult) ? null : rawConfigResult.value;

    const modelConfig = resolveModelConfig('pre-recon', rawConfig);
    const providerResult = await validateProvider(modelConfig);

    if (isErr(providerResult)) {
      const classified = classifyErrorForTemporal(
        new PentestError(providerResult.error, 'config', false, {}, ErrorCode.PROVIDER_AUTH_FAILED)
      );
...
export async function assembleReportActivity(input: ActivityInput): Promise<void> {
  ...
  try {
    ...
    logger.warn(`Error assembling final report: ${errorMessage}`);
  }
}
...
export async function injectReportMetadataActivity(input: ActivityInput): Promise<void> {
  ...
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    logger.warn(`Error injecting model into report: ${errorMessage}`);
  }
}
...
export async function logWorkflowComplete(
  ...
  const sessionData = await auditSession.getMetrics();
