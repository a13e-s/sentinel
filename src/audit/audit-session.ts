...
import { MetricsTracker, type SessionData } from './metrics-tracker.js';
...
export class AuditSession {
  ...
  constructor(sessionMetadata: SessionMetadata) {
    ...
    if (!this.sessionId) {
      throw new Error(`sessionMetadata.id is required [${ErrorCode.CONFIG_VALIDATION_FAILED}]`);
    }
    if (!this.sessionMetadata.webUrl) {
      throw new Error(`sessionMetadata.webUrl is required [${ErrorCode.CONFIG_VALIDATION_FAILED}]`);
    }
  ...
  async logEvent(eventType: string, eventData: unknown): Promise<void> {
    if (!this.currentLogger) {
      throw new Error(
        `No active logger. Call startAgent() first. [${ErrorCode.AGENT_EXECUTION_FAILED}]`
      );
    }
   ...
   */
  async getMetrics(): Promise<SessionData> {
    await this.ensureInitialized();
